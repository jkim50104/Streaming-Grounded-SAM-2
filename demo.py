...
def extract(query, model):
    with open("llm/openie.txt", "r") as file:
        ie_prompt = file.read()
    text = model.generate(ie_prompt.format_map({"query": query}))
...
async def main(model="gpt-4o-2024-05-13", use_llm=False):
    ...
    cap = cv2.VideoCapture("/home/jokim/projects/openvla-oft/rollouts/2025_04_14/2025_04_14-21_20_57--openvla--episode=4--success=True--task=put_the_banana_to_the_yellow_plate.mp4")
    ...
    text = "a black two finger gripper. " #"a banana. a yellow plate."
    results = None
    boxes = None
    ...
    if use_llm:
        extracted_objects = extract(query, llm)
        print(f"LLM Response: {extracted_objects}")
        if_init = False
    else:
        extracted_objects = "a banana. a yellow plate."
    text += extracted_objects
